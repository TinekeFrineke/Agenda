...
void Settings::FillFrom(const utils::Inifile& inifile)
{
    std::stringstream stream(inifile.get("General", "StartDate"));
    stream >> m_DefaultStartDate;

    stream.str(inifile.get("General", "EndDate"));
    stream >> m_DefaultEndDate;

    m_AgendaType = ToType(inifile.get("General", "Type", "Combined"));

    unsigned int count(std::atoi(inifile.get("Items", "Count").c_str()));
    for (unsigned int i = 0; i < count; ++i) {
        char number[10];
        sprintf_s(number, "%u", i);
        stream.str(inifile.get("Items", number));
        if (number[0] != 0) {
...
void Settings::WriteTo(utils::Inifile& inifile)
{
    inifile.set("General", "StartDate", m_DefaultStartDate.String());
    inifile.set("General", "EndDate", m_DefaultEndDate.String());
    inifile.set("General", "Type", ToString(m_AgendaType));


    char number[10];
    sprintf_s(number, "%zu", m_DefaultActivities.size());
    inifile.set("Items", "Count", number);

    for (unsigned int i = 0; i < m_DefaultActivities.size(); ++i) {
        sprintf_s(number, _T("%u"), i);
        inifile.set("Items", number, m_DefaultActivities[i].m_Description
